...
def compare_models(prompt: str) -> dict:
    ...
    output_tokens = max(1, len(gpt4o_response.split()))
    cost_per_token = PRICING_PER_1K_TOKENS["gpt-4o"]["output"] / 1000
    gpt4o_cost_estimate = output_tokens * cost_per_token
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        assistant_reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content if getattr(chunk.choices[0].delta, "content", None) else ""
...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        user_tokens = count_tokens(user_msg, model=OPENAI_MODEL)
        assistant_tokens = count_tokens(reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for idx, row in enumerate(rows):
        padded = " | ".join(row[i].ljust(col_widths[i]) for i in range(len(headers)))
        lines.append(padded)
        if idx == 0:
